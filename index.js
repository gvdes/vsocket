 ...
 */
 ...
 preventa.on('connection',pv=>{
     console.log("nueva conexion en PREVENTA");
 
     pv.on('index',gdata=>{
         console.log(`${gdata.me.names} ${gdata.me.surname_pat} [${gdata.me.nick}] de ${gdata.workpoint.name} [${gdata.workpoint.alias}] ha ingresado al home de Preventa`);
         console.log('\n');
     });
     ...
     pv.on('joinat',gdata=>{
         ...
         preventa.in(room).emit( 'joinprev', {user:user,notify:`${msg} se ha unido al room ${room}` } );
         preventa.in(roomdash).emit( 'joinprevwrh', {user:user,notify:`${msg} se ha unido al room ${roomdash}` } );
     });
 ...
 resurtidos.on('connection',dashboard=>{
     console.log("Nueva conexion a RESURTIDO");
